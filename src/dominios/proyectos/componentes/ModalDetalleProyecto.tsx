...
import { projectLabels } from '../datosProyectos'
import { clasesBoton } from '../../../componentes/ui/Boton'

export type PropsModalDetalleProyecto = {
  project: Project
  onClose: () => void
}

export const ModalDetalleProyecto = ({
  project,
  onClose,
}: PropsModalDetalleProyecto) => {
  useEffect(() => {
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-end">
            <button
              type="button"
              onClick={onClose}
              className={clasesBoton({ variant: 'secondary', size: 'sm' })}
            >
              {projectLabels.close}
            </button>
          </div>
